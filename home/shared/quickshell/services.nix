...

{
  ...
  systemd.user.services = {
    ...
    wayland-idle-inhibitor = {
      ...
      Service = {
        Type = "simple";
        ExecStart = "${config.home.homeDirectory}/.local/state/quickshell/.venv/bin/python ${config.home.homeDirectory}/Workspaces/Config/dots-hyprland/.config/quickshell/ii/scripts/wayland-idle-inhibitor.py";
        Restart = "on-failure";
        RestartSec = 1;
